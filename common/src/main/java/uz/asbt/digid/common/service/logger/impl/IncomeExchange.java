...
@Service("incomeExchange")
public class IncomeExchange implements Exchangable<Income> {

    private final Logger logger = LoggerFactory.getLogger(getClass());

    @Value("${rabbit.income.routing.key}")
    private String routingKey;

    @Value("${rabbit.income.exchange}")
    private String exchange;

    private RabbitTemplate rabbitTemplate;

    @Autowired
    public IncomeExchange(RabbitTemplate rabbitTemplate) {
        this.rabbitTemplate = rabbitTemplate;
    }

    @Async
    @Override
    public void sendToQueue(Income object) {
        try {
